  ...
  for (const network of networks) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        legacyAddresses[network][contractName] = {
...
function processIgnitionDeployments(): { abis: GroupedABIs; addresses: GroupedAddresses } {
  ...
  if (!fs.existsSync(ignitionPath)) {
    ...
    return { abis: groupedAbis, addresses: groupedAddresses };
  }
    ...
    if (fs.existsSync(addressesPath)) {
      ...
      for (const [ignitionKey, address] of Object.entries(deployedAddresses)) {
        ...
        if (!allContracts.has(fullKey)) {
          allContracts.set(fullKey, { category, address, network: chainDir });

          // Read ABI from artifacts (only need to do this once)
          ...
          if (fs.existsSync(artifactPath)) {
            try {
              ...
              if (artifact.abi && artifact.abi.length > 0) {
                const contractInfo = allContracts.get(fullKey)!;
                contractInfo.abi = artifact.abi;
              }
            } catch (error) {
          ...
          // Verify address consistency across networks
...
function main(): void {
  ...
  // Process ignition deployments
  const { abis: currentAbis, addresses: currentAddresses } = processIgnitionDeployments();
  ...
  // Write output files
  writeTypeScriptFiles(currentAbis, currentAddresses, legacyAbis, legacyAddresses);
