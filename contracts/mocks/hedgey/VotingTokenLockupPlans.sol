...
contract VotingVault {
    ...
    constructor(address _token, address beneficiary) {
        ...
        token = _token;
        address existingDelegate = IGovernanceToken(token).delegates(
            beneficiary
        );
        if (existingDelegate != address(0))
            IGovernanceToken(token).delegate(existingDelegate);
    ...
    function delegateTokens(address delegatee) external onlyController {
        ...
        require(
            balanceCheck == IERC20(token).balanceOf(address(this)),
            "balance error"
        );
    }
...
contract VotingTokenLockupPlans is
    PlanDelegator,
    LockupStorage,
    ReentrancyGuard,
    URIAdmin
{
    ...
    function setupVoting(
        uint256 planId
    ) external nonReentrant returns (address votingVault) {
