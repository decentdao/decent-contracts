...
describe("Child Multisig DAO with Azorius Parent", () => {
  ...
  beforeEach(async () => {
    ...
    mockERC721 = await new MockERC721(deployer).deploy();
    ...
    freezeVotingMastercopy = await new ERC721FreezeVoting__factory(
      deployer
    ).deploy();
    ...
    const freezeVotingSetupData =
      // eslint-disable-next-line camelcase
      ERC721FreezeVoting__factory.createInterface().encodeFunctionData(
        "setUp",
        [
          abiCoder.encode(
