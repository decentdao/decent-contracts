...
describe('LinearERC721VotingV1', () => {
  ...
  async function deployLinearERC721Voting(
    ...
    return LinearERC721VotingV1__factory.connect(predictedLinearERC721VotingAddress, strategyOwner);
  }
  ...
  describe('Initialization', () => {
    it('should initialize with correct parameters', async () => {
      ...
      expect(await linearERC721Voting.azoriusModule()).to.equal(proposalInitializer);
  ...
  describe('Edge Cases', () => {
    ...
    it('should emit ProposalInitialized event when initializing a proposal', async () => {
      ...
      // Get current block to calculate expected end block
      const currentBlock = await time.latestBlock();

      // +1 because the proposal will be in the next block, and that block's number will be increased by one
      const expectedEndBlock = currentBlock + 1 + VOTING_PERIOD;
  ...
  describe('Block-based voting', () => {
    ...
    it('should enforce voting end by block number', async () => {
      // Get the voting end block number
    ...
    it('should determine proposal state based on block number', async () => {
      ...
      // Get voting end block number
      const [, , , , endBlock] = await linearERC721Voting.getProposalVotes(proposalId);

      // Before advancing time, proposal should not be passed
      void expect(await linearERC721Voting.isPassed(proposalId)).to.be.false;

      // Advance time to AFTER the voting end block number (critical for isPassed)
      await mineUpTo(endBlock + 1n);

      // Now the proposal should pass (past end block number + meets quorum + meets basis)
      void expect(await linearERC721Voting.isPassed(proposalId)).to.be.true;
    });
    ...
    it('should handle exact boundary conditions for block numbers', async () => {
      ...
      // Advance time to 3 seconds before the voting end block number
      await mineUpTo(endBlock - 3n);
